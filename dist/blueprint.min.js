...
bp=blueprint(name,schema.schema)}else{bp=blueprint(name,schema)}if(bp.err){throw bp.err}var cleanMessage=function cleanMessage(key,message){return message.replace("Invalid ".concat(bp.name,": "),"").replace(/expected `/g,"expected `".concat(key,"."))};registerType(bp.name,function(_ref){var key=_ref.key,value=_ref.value;var result=bp.validate(value);if(result.err){result.err.message=cleanMessage(key,result.err.message)}return result});return bp};/**
     ...
     */var optional=function optional(comparator){var options={};var validator;if(is.function(comparator)){validator=normalIsValid(comparator)}else if(is.regexp(comparator)){validator=normalIsValid(validators.expression(comparator))}else{validator=validators[comparator]}var output=function output(context){var value=context.value;if(is.nullOrUndefined(value)){return is.defined(options.defaultValue)?{value:options.defaultValue}:{value:value}}else{return validator(context)}};output.withDefault=function(defaultValue){options.defaultValue=defaultValue;return output};return output};return{blueprint:blueprint,registerValidator:registerValidator,registerType:registerType,registerBlueprint:registerBlueprint,registerExpression:registerExpression,optional:optional,// below are undocumented / subject to breaking changes
