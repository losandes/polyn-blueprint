...
configurable:false});module.exports={name:"blueprint",factory:function factory(is){'use strict';var validators={};var ValueOrError=function ValueOrError(input){_classCallCheck(this,ValueOrError);this.err=input.err||null;this.value=is.defined(input.value)?input.value:null;if(is.array(input.messages)){this.messages=input.messages}else if(input.err){this.messages=[input.err.message]}else{this.messages=null}Object.freeze(this)};var ValidationContext=function ValidationContext(input){_classCallCheck(this,ValidationContext);this.key=input.key;this.value=input.value;this.input=input.input;this.root=input.root;this.output=input.output;this.schema=input.schema};var Blueprint=function Blueprint(input){_classCallCheck(this,Blueprint);this.name=input.name;this.schema=input.schema;this.validate=input.validate;Object.freeze(this)};/**
     ...
     */var tryMakeFromProto=function tryMakeFromProto(input){try{return Object.create(Object.getPrototypeOf(input))}catch(e){return{}}};var decorateError=function decorateError(_ref){var key=_ref.key,err=_ref.err;err.blueprint={};err.blueprint.invalid={};err.blueprint.invalid[key]=err.message;return{key:key,err:err}};/**
     ...
     */var validate=function validate(name,schema){return function(input,root){var outcomes=Object.keys(schema).reduce(function(output,key){var keyName=root?"".concat(name,".").concat(key):key;if(is.object(schema[key])){var child=validate("".concat(keyName),schema[key])(input[key],root||input);if(child.err){output.validationErrors=output.validationErrors.concat(child.err.keyErrList)}output.value[key]=child.value;return output}var validator;if(is.function(schema[key])){validator=normalIsValid(schema[key])}else if(is.regexp(schema[key])){validator=normalIsValid(validators.expression(schema[key]))}else{validator=validators[schema[key]]}if(is.not.function(validator)){output.validationErrors.push(decorateError({key:key,err:new Error("I don't know how to validate ".concat(schema[key]))}));return output}var context=new ValidationContext({key:"".concat(keyName),value:input&&input[key],input:input,root:root||input,output:output.value,schema:schema});var result=validator(context,makeDefaultErrorMessage(context));if(result&&result.err){output.validationErrors.push(decorateError({key:context.key,err:result.err}));return output}output.value[key]=result?result.value:input[key];return output},{/* output */validationErrors:[],value:tryMakeFromProto(input)});// /reduce
if(outcomes.validationErrors.length){var messages=outcomes.validationErrors.map(function(errGroup){return errGroup.err.message});var error=new Error("Invalid ".concat(name,": ").concat(messages.join(", ")));error.keyErrList=outcomes.validationErrors;error.blueprint={};error.blueprint.invalids=outcomes.validationErrors.reduce(function(prev,errGroup){return _objectSpread(_objectSpread({},prev),{},_defineProperty({},errGroup.key,errGroup.err.message))},{});error.blueprint.errors=outcomes.validationErrors.map(function(validationErr){return validationErr.err});return new ValueOrError({err:error,messages:messages})}return new ValueOrError({value:outcomes.value})}};// /validate
/**
...
bp=blueprint(name,schema.schema)}else{bp=blueprint(name,schema)}var cleanMessage=function cleanMessage(key,message){return message.replace("Invalid ".concat(bp.name,": "),"").replace(/expected `/g,"expected `".concat(key,"."))};registerType(bp.name,function(_ref2){var key=_ref2.key,value=_ref2.value;var result=bp.validate(value);if(result.err){result.err.message=cleanMessage(key,result.err.message)}return result});return bp};/**
     ...
     */var registerExpression=function registerExpression(name,expression){if(is.not.string(name)||is.not.regexp(expression)&&is.not.string(expression)){throw new Error("registerExpression requires a name {string}, and an expression {expression}")}var regex=is.string(expression)?new RegExp(expression):expression;return registerType(name,function(_ref3){var key=_ref3.key,value=_ref3.value;return regex.test(value)===true?new ValueOrError({value:value}):new ValueOrError({err:new Error("expected `".concat(key,"` to match ").concat(regex.toString()))})})};var getValidators=function getValidators(){return _objectSpread({},validators)};var getValidator=function getValidator(name){if(!validators[name]){return}return _objectSpread({},validators[name])};var comparatorToValidator=function comparatorToValidator(comparator){var validator;if(is.function(comparator)){validator=normalIsValid(comparator)}else if(is.regexp(comparator)){validator=normalIsValid(validators.expression(comparator))}else{validator=validators[comparator]}return validator};/**
     ...
     */var optional=function optional(comparator){var defaultVal;var from;var validator=comparatorToValidator(comparator);var valueOrDefaultValue=function valueOrDefaultValue(value){if(is.function(defaultVal)){return{value:defaultVal()}}else if(is.defined(defaultVal)){return{value:defaultVal}}else{return{value:value}}};var output=function output(ctx){var context;if(from){context=_objectSpread(_objectSpread({},ctx),{value:from(ctx)})}else{context=ctx}var _context=context,value=_context.value;if(is.nullOrUndefined(value)){return valueOrDefaultValue(value)}else{return validator(context)}};/**
       ...
       */output.from=function(callback){if(is.function(callback)){from=callback}return output};/**
       ...
       */output.withDefault=function(defaultValue){defaultVal=defaultValue;return output};return output};/**
     * Fluent interface to support optional function based validators
     * (i.e. like gt, lt, range, custom), and to use default values when
     * the value presented is null, or undefined.
     * @param {any} comparator - the name of the validator, or a function that performs validation
     */var required=function required(comparator){var from;var validator=comparatorToValidator(comparator);var output=function output(ctx){var context;if(from){context=_objectSpread(_objectSpread({},ctx),{value:from(ctx)})}else{context=ctx}return validator(context)};/**
       * A value factory for producing a value, given the constructor context
       * @param {function} callback - a callback function that accepts IValidationContext and produces a value
       */output.from=function(callback){if(is.function(callback)){from=callback}return output};return output};return{blueprint:blueprint,registerValidator:registerValidator,registerType:registerType,registerBlueprint:registerBlueprint,registerExpression:registerExpression,optional:optional,required:required,// below are undocumented / subject to breaking changes
registerInstanceOfType:registerInstanceOfType,registerArrayOfType:registerArrayOfType,getValidators:getValidators,getValidator:getValidator}}};module.exports={name:"is",factory:function factory(){'use strict';var is={getType:undefined,defined:undefined,nullOrUndefined:undefined,function:undefined,func:undefined,promise:undefined,asyncFunction:undefined,asyncFunc:undefined,object:undefined,array:undefined,string:undefined,boolean:undefined,date:undefined,regexp:undefined,number:undefined,nullOrWhitespace:undefined,decimal:undefined,primitive:undefined,arrayOf:undefined,not:{defined:undefined,nullOrUndefined:undefined,function:undefined,func:undefined,promise:undefined,asyncFunction:undefined,asyncFunc:undefined,object:undefined,array:undefined,string:undefined,boolean:undefined,date:undefined,regexp:undefined,number:undefined,nullOrWhitespace:undefined,decimal:undefined,primitive:undefined,arrayOf:undefined}};var primitives=["boolean","null","undefined","number","bigint","string","symbol"];/**
     ...
     * date, regexp, object)
     */is.getType=function(obj){return Object.prototype.toString.call(obj).replace(/(^\[object )|(\]$)/g,"").toLowerCase()};is.defined=function(obj){return is.getType(obj)!=="undefined"};is.not.defined=function(obj){return is.defined(obj)===false};is.nullOrUndefined=function(obj){return is.not.defined(obj)||obj===null};is.not.nullOrUndefined=function(obj){return is.nullOrUndefined(obj)===false};is.not.nullOrWhitespace=function(str){if(typeof str==="undefined"||str===null){return false}else if(Array.isArray(str)){return true}// ([^\s]*) = is not whitespace
// /^$|\s+/ = is empty or whitespace
return /([^\s])/.test(str)};is.nullOrWhitespace=function(str){return is.not.nullOrWhitespace(str)===false};is.function=function(obj){var type=is.getType(obj);return type==="function"||type==="asyncfunction"||type==="promise"};is.func=is.function;// typescript support
is.not.function=function(obj){return is.function(obj)===false};is.not.func=is.not.function;// typescript support
is.promise=function(obj){var type=is.getType(obj);return type==="asyncfunction"||type==="promise"};is.not.promise=function(obj){return is.promise(obj)===false};is.asyncFunction=function(obj){return is.promise(obj)};is.asyncFunc=is.asyncFunction;// consistency for typescript
is.not.asyncFunction=function(obj){return is.asyncFunction(obj)===false};is.not.asyncFunc=is.not.asyncFunction;// consistency for typescript
...
return padded.toFixed(places)==="".concat(+num)};is.not.decimal=function(val,places){return is.decimal(val,places)===false};is.primitive=function(input){return primitives.indexOf(is.getType(input))>-1};is.not.primitive=function(input){return is.primitive(input)===false};is.arrayOf=function(type){if(!is[type]){throw new Error("is does not support evaluation of {".concat(type,"}"))}return function(input){return input.find(function(v){return is.not[type](v)})===undefined}};is.not.arrayOf=function(type){if(!is[type]){throw new Error("is does not support evaluation of {".concat(type,"}"))}return function(input){return is.arrayOf(type)(input)===false}};return is}};module.exports={name:"numberValidators",factory:function factory(is){'use strict';var makeErrorMessage=function makeErrorMessage(options){return"expected `".concat(options.key,"` to be ").concat(options.comparator," ").concat(options.boundary)};var gt=function gt(min){if(is.not.number(min)){throw new Error("gt requires a minimum number to compare values to")}return function(_ref4){var key=_ref4.key,value=_ref4.value;if(is.number(value)&&value>min){return{err:null,value:value}}return{err:new Error(makeErrorMessage({key:key,comparator:"greater than",boundary:min})),value:null}}};var gte=function gte(min){if(is.not.number(min)){throw new Error("gte requires a minimum number to compare values to")}return function(_ref5){var key=_ref5.key,value=_ref5.value;if(is.number(value)&&value>=min){return{err:null,value:value}}return{err:new Error(makeErrorMessage({key:key,comparator:"greater than, or equal to",boundary:min})),value:null}}};var lt=function lt(max){if(is.not.number(max)){throw new Error("lt requires a maximum number to compare values to")}return function(_ref6){var key=_ref6.key,value=_ref6.value;if(is.number(value)&&value<max){return{err:null,value:value}}return{err:new Error(makeErrorMessage({key:key,comparator:"less than",boundary:max})),value:null}}};var lte=function lte(max){if(is.not.number(max)){throw new Error("lte requires a maximum number to compare values to")}return function(_ref7){var key=_ref7.key,value=_ref7.value;if(is.number(value)&&value<=max){return{err:null,value:value}}return{err:new Error(makeErrorMessage({key:key,comparator:"less than, or equal to",boundary:max})),value:null}}};var range=function range(options){if(!options){throw new Error("You must specify a range")}else if(is.not.number(options.gt)&&is.not.number(options.gte)){throw new Error("You must specify `gt`, or `gte` {number} when defining a range")}else if(is.not.number(options.lt)&&is.not.number(options.lte)){throw new Error("You must specify `lt`, or `lte` {number} when defining a range")}var gtExpression=options.gt?gt(options.gt):gte(options.gte);var ltExpression=options.lt?lt(options.lt):lte(options.lte);return function(input){var ltOutcome=ltExpression(input);if(ltOutcome.err){return ltOutcome}return gtExpression(input)}};var optional=function optional(comparator){return function(input){var validator=comparator(input);return function(context){var value=context.value;if(is.nullOrUndefined(value)){return{value:value}}else{return validator(context)}}}};return{gt:gt,gte:gte,lt:lt,lte:lte,range:range,// backward compatibility - can be removed in v3
__optional:{gt:optional(gt),gte:optional(gte),lt:optional(lt),lte:optional(lte),range:optional(range)}}}};module.exports={name:"registerCommonTypes",factory:function factory(is,Blueprint){'use strict';var registerType=Blueprint.registerType;var types=["function","asyncFunction","promise","object","array","boolean","date","number","decimal","regexp","primitive"// 'string' registered separately, below
];var errorMessage=function errorMessage(type){return function(key,value){return"expected `".concat(key,"` {").concat(is.getType(value),"} to be {").concat(type,"}")}};types.forEach(function(type){registerType(type,function(_ref8){var key=_ref8.key,value=_ref8.value;return is[type](value)?{err:null,value:value}:{err:new Error(errorMessage(type)(key,value))}})});registerType("string",function(_ref9){var key=_ref9.key,value=_ref9.value;if(is.string(value)){var trimmed=value.trim();if(trimmed.length){return{value:trimmed}}return{err:new Error("expected `".concat(key,"` {").concat(is.getType(value),"} to not be an empty string"))}}else{return{err:new Error(errorMessage("string")(key,value))}}});registerType("any",function(_ref10){var key=_ref10.key,value=_ref10.value;return is.not.nullOrUndefined(value)?{err:null,value:value}:{err:new Error(errorMessage("any")(key,value))}})}};module.exports={name:"registerDecimals",factory:function factory(is,Blueprint){'use strict';var registerValidator=Blueprint.registerValidator;// support up to 15 decimal places for decimal precision
var _loop=function _loop(i){registerValidator("decimal:".concat(i),function(_ref11){var key=_ref11.key,value=_ref11.value;return is.decimal(value,i)?{err:null,value:value}:{err:new Error("expected `".concat(key,"` to be a {decimal} with ").concat(i," places")),value:null}});registerValidator("decimal:".concat(i,"?"),function(_ref12){var key=_ref12.key,value=_ref12.value;if(is.nullOrUndefined(value)){return{err:null,value:value}}else if(is.decimal(value,i)){return{err:null,value:value}}else{return{err:new Error("expected `".concat(key,"` to be a {decimal} with ").concat(i," places")),value:null}}})};for(var i=1;i<=15;i+=1){_loop(i)}}};module.exports={name:"registerExpressions",factory:function factory(Blueprint){'use strict';var registerValidator=Blueprint.registerValidator;registerValidator("expression",function(regex){return function(_ref13){var key=_ref13.key,value=_ref13.value;return regex.test(value)===true?{value:value}:{err:new Error("expected `".concat(key,"` to match ").concat(regex.toString()))}}})}};var is=module.factories.is();var numberValidators=module.factories.numberValidators(is);var blueprint=module.factories.blueprint(is);// backward compatibility - can be removed in v3
Object.keys(numberValidators.__optional).forEach(function(key){blueprint.optional[key]=numberValidators.__optional[key]});delete numberValidators.__optional;root.polyn=root.polyn||{};root.polyn.blueprint=Object.freeze(Object.assign({is:is},numberValidators,blueprint));module.factories.registerCommonTypes(is,root.polyn.blueprint);module.factories.registerDecimals(is,root.polyn.blueprint);module.factories.registerExpressions(root.polyn.blueprint);// we don't need these anymore
