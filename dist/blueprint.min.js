...
bp=blueprint(name,schema.schema)}else{bp=blueprint(name,schema)}if(bp.err){throw bp.err}var cleanMessage=function cleanMessage(key,message){return message.replace("Invalid ".concat(bp.name,": "),"").replace(/expected `/g,"expected `".concat(key,"."))};registerType(bp.name,function(_ref){var key=_ref.key,value=_ref.value;var result=bp.validate(value);if(result.err){result.err.message=cleanMessage(key,result.err.message)}return result});return bp};/**
     ...
     * the value presented is null, or undefined.
     * @param {any} comparator - the name of the validator, or a function that performs validation
     */var required=function required(comparator){var from;var validator=comparatorToValidator(comparator);var output=function output(ctx){var context;if(from){context=_objectSpread({},ctx,{},{value:from(ctx)})}else{context=ctx}return validator(context)};/**
       * A value factory for producing a value, given the constructor context
       * @param {function} callback - a callback function that accepts IValidationContext and produces a value
...
is.promise=function(obj){var type=is.getType(obj);return type==="asyncfunction"||type==="promise"};is.not.asyncFunction=function(obj){return is.promise(obj)===false};is.asyncFunction=function(obj){return is.promise(obj)};is.asyncFunc=is.asyncFunction;// consistency for typescript
